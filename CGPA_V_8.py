#Rns Method intuition:

a = 10.466
def get_round_any(a, step):
    a_floor = a // step
    if a > a_floor:
        return a_floor*step + step
print(get_round_any(a, 1))
def cal_mark_req(mark, step):
   remainder = mark % step
   return remainder + 1
print(cal_mark_req(117, 7.5))

def calc_rns_percentage(a, step, total):
    mark_rounded = get_round_any(a, step)
    return round(mark_rounded/total, 2)
print(calc_rns_percentage(117, 7.5, 150))
...
#the dict: {'math': {'ia': [["value:", value], [9, 10],[8,10]], 'lab':.., 'credit': [[3]]}}
