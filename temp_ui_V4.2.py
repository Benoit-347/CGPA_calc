...
class ResultScreen(Screen):
    ...
    def on_pre_enter(self, *args):
        # Clear the layout in case of re-entry
        self.layout.clear_widgets()

        # Add results header
        self.layout.add_widget(Label(text="Results", font_size=24))

        # Dynamically populate results
        for i, subject in enumerate(self.subject_list):
            if i < len(self.result_list):  # Avoid out-of-range access
                self.layout.add_widget(Label(text=f"{subject}: {self.result_list[i]}"))
            else:
                self.layout.add_widget(Label(text=f"{subject}: No data"))
